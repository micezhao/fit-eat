...
public class MerchantBiz {
	 ...
	 * @return
	 */
	public Merchant enterApply(MerchantRequest request) {
	...
	private Merchant updateMerchantById(MerchantRequest request) {
		...
		return mongoTemplate.findAndModify(new Query().addCriteria(new Criteria(FieldConstants.MERCHANT_ID).is(request.getMerchantId())), 
									update, 
									FindAndModifyOptions.options().returnNew(true),
									Merchant.class);
	}
	
	private  Merchant newMerchantSave(MerchantRequest request) {
		Merchant dest = new Merchant();
	 ...
	 * @return
	 */
	public Merchant rejectApply(MerchantRequest request) {
		Query query = new Query();
		query.addCriteria(new Criteria(FieldConstants.MERCHANT_ID).is(request.getMerchantId()));
		Merchant updatedRecord = mongoTemplate.findAndModify(query,
				new Update()
					.set(FieldConstants.VERIFY_STATUS, MerchantStatus.UN_VERIFIED.getCode())
	...
	public Merchant approveApply(String id) {
		...
		Merchant updatedRecord = mongoTemplate.findAndModify(query,
				new Update().set(FieldConstants.VERIFY_STATUS, MerchantStatus.VERIFIED.getCode())
						.set(FieldConstants.OPERATION_STATUS, MerchantStatus.OPENING.getCode())
						.set(FieldConstants.MDT, LocalDateTime.now()),
				FindAndModifyOptions.options().returnNew(true), Merchant.class);
		return updatedRecord;
	}
	 ...
	 * @param command 操作命令
	 */
	public void operateById(String id, String command) {
		
		Query query = new Query();
		query.addCriteria(new Criteria(FieldConstants.MERCHANT_ID).is(id));
		Merchant item= mongoTemplate.findOne(query,  Merchant.class);
		...
		MerchantStatus targetStatus = MerchantStatus.getEnumByCode(command);
	 ...
	 * @return
	 */
	public Merchant queryById(String id) {
