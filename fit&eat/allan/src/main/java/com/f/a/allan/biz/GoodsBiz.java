...
public class GoodsBiz {
	...
	public boolean deduct(String goodsId, int deduction) {
		...
		if (redisHasKey) {
			...
			remainStock = mongoTemplate.findOne(query, GoodsItem.class).getStock().intValue();
		}
	...
	public boolean deductGoodsStockById(String goodsId,int remainStock) {
		try {
			Query query = new Query();
			query.addCriteria(new Criteria(FieldConstants.GOODS_ID).is(goodsId));
			GoodsItem item = mongoTemplate.findOne(query, GoodsItem.class);
			...
			//TODO  先更新 redis 同时 异步更新 mongodb
