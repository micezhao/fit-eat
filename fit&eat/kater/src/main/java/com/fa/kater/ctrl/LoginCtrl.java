...
public class LoginCtrl {
	...
	public ResponseEntity<AuthBo> getAuthCode(@PathVariable("code") String code,
			@PathVariable("loginType") String loginType, HttpSession httpSession) {
		// 2.获取授权结果
	...
	// 登录
	@GetMapping("thirdPart/{agentId}/{loginType}/{thirdAuthId}")
	public ResponseEntity<Object> login(@PathVariable(value = "agentId") String agentId,
			...
			@PathVariable(value = "thirdAuthId") String thirdAuthId, HttpSession session) {
		UserAgent userAgent = null;
		Map<String, Object> resultMap = new HashMap<String, Object>();
		try {
			// 生成UserAgent
			userAgent = loginBiz.generateUserAgent(agentId,loginType, thirdAuthId);
			// 需要对登录信息表进行操作
